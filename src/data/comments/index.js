...
export const batchGetComments = async (db, keys) => {
  ...
  return _.sortBy(comments, comment => keys.indexOf(comment.id)).map(
    comment => {
      return {
        id: comment.id,
        body: comment.content
...
export const getCommentLoader = db =>
  new DataLoader(keys => batchGetComments(db, keys));
